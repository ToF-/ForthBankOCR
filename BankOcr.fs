...
CREATE ALT-ACCOUNT 9 9 * ALLOT
VARIABLE ALT-ACCOUNTS

: FIND-MISSING-BAR ( -- )
    0 ALT-ACCOUNTS !
    OCR-BITS C@ DUP >R 0 ADD-MISSING-BIT OCR-BITS C!
    OCR-BITS ALT-ACCOUNT 9 FIND-DIGITS
    OCR-BITS 9 DUMP
    ALT-ACCOUNT 9 TYPE CR
    R> OCR-BITS C!
    ALT-ACCOUNT 9 2DUP 
    ILLEGIBLE? 0= -ROT CHECKSUM? AND IF 1 ALT-ACCOUNTS +! THEN ; 
