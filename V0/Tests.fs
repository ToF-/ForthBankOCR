...
: CHECKS
    ...
    [ DECIMAL ]
    ASSERT( 0 0 OCR-ADDRESS OCR-BUFFER  = )
    ASSERT( 0 2 OCR-ADDRESS OCR-BUFFER 2 +  = )
    ASSERT( 0 3 OCR-ADDRESS OCR-BUFFER 27 + = )
    ASSERT( 0 8 OCR-ADDRESS OCR-BUFFER 56 + = )
    ASSERT( 8 6 OCR-ADDRESS OCR-BUFFER 78 + = )
    ASSERT( 8 7 OCR-ADDRESS OCR-BUFFER 79 + = )
    ASSERT( 8 8 OCR-ADDRESS OCR-BUFFER 80 + = )
    ASSERT( 0 OCR>PATTERN [ BINARY ] 010101111 = [ DECIMAL ] )
    ASSERT( 1 OCR>PATTERN [ BINARY ] 000001001 = [ DECIMAL ] )
    ASSERT( 8 OCR>PATTERN [ BINARY ] 010111111 = [ DECIMAL ] )
    ASSERT( [ BINARY ] 011111111 [ DECIMAL ] PATTERN>DIGIT NOT-FOUND =  )
    ASSERT( [ BINARY ] 000111001 [ DECIMAL ] PATTERN>DIGIT [CHAR] 4 =  )
